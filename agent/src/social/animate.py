...
.anim .buck { animation: enter 1.1s cubic-bezier(.2,1.2,.4,1) both; animation-delay: calc(0s - 1s * var(--t)); }
@keyframes enter { from { transform: translateY(140px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
.anim .buck svg { animation: bob 3.4s ease-in-out infinite; animation-delay: calc(1.1s - 1s * var(--t)); transform-origin: 50% 100%; }
@keyframes bob { 0%, 100% { transform: translateY(0) scale(1, 1); } 50% { transform: translateY(-10px) scale(.985, 1.015); } }
.anim .buck svg .eyes { transform-box: view-box; transform-origin: 200px 250px; animation: blink 4.2s infinite; animation-delay: calc(1.6s - 1s * var(--t)); }
@keyframes blink { 0%, 91%, 100% { transform: scaleY(1); } 94% { transform: scaleY(.06); } 97% { transform: scaleY(1); } }
.anim .buck svg.pose-wave .arm-R, .anim .buck svg.pose-cheer .arm-R, .anim .buck svg.pose-cheer .arm-L, .anim .buck svg.pose-point .arm-R { transform-box: view-box; animation: wave 1.6s ease-in-out infinite; animation-delay: calc(1.1s - 1s * var(--t)); }
...
.anim .buck svg .sweat { animation: drop 2.2s ease-in infinite; animation-delay: calc(1.4s - 1s * var(--t)); }
@keyframes drop { 0% { transform: translateY(0); opacity: 1; } 80% { opacity: 1; } 100% { transform: translateY(70px); opacity: 0; } }
.anim .buck svg .thought { animation: pulse 2.2s ease-in-out infinite; animation-delay: calc(1.4s - 1s * var(--t)); }
@keyframes pulse { 0%, 100% { opacity: .4; } 50% { opacity: 1; } }
.anim .buck svg.pose-hold .prop, .anim .buck svg.pose-hold .hand { transform-box: view-box; transform-origin: 200px 350px; animation: hold 3.4s ease-in-out infinite; animation-delay: calc(1.1s - 1s * var(--t)); }
@keyframes hold { 0%, 100% { transform: translateY(0) rotate(0deg); } 50% { transform: translateY(-6px) rotate(-1.5deg); } }
.anim .bubble { animation: pop .7s cubic-bezier(.2,1.4,.4,1) both; animation-delay: calc(1.2s - 1s * var(--t)); transform-origin: 20% 100%; }
@keyframes pop { from { transform: scale(0); opacity: 0; } to { transform: scale(1); opacity: 1; } }
.anim .cap, .anim .cap.bottom { animation: fade .6s ease both; animation-delay: calc(.3s - 1s * var(--t)); }
@keyframes fade { from { opacity: 0; } to { opacity: 1; } }
.anim .fx { animation: slam .5s cubic-bezier(.2,1.6,.4,1) both; animation-delay: calc(2.0s - 1s * var(--t)); }
...
def render_frames(spec: dict, slug: str, out_dir: Path, seconds: float = 5.2, cover: bool = True, cta_seconds: float = 3.0, still_seconds: float = 4.5) -> list[tuple[Path, float]]:
    ...
    seq: list[tuple[Path, float]] = [(stills[0], 3.0)] if cover else []
...
def animate_clip(spec: dict, out_slug: str, music: Path | None = None, preview_bed: bool = False, seconds: float = 4.6) -> dict:
    """A short tip clip (10-15 s): no cover, one or two animated panels, a lesson still, the follow card."""
    work = IMG_DIR / "anim" / out_slug
    seq = render_frames(spec, out_slug, work, seconds=seconds, cover=False, cta_seconds=2.4, still_seconds=3.6)
    total = sum(d for _, d in seq)
    needs_audio = False
    if music is None:
        music = pick_music()
